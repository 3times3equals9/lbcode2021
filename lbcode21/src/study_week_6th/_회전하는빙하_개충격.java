...
public class _회전하는빙하_개충격 {
	...
	public static void clock(int sr, int sc, int len) {
		...
		if(len == 2) {
			...
			int unit = len/2;
			for(int st_r=0; st_r<len; st_r=st_r+unit) {
				for(int st_c=0; st_c<len; st_c=st_c+unit) {
					//단위
					for(int r=0; r<unit; r++) {
						for(int c=0; c<unit; c++) {
							//arr[r][c] = copy[(st_c)+r][(unit-st_r)+c];
							arr[st_c + r][(unit-st_r) + c] = copy[st_r + r][st_c + c];
						}
					}
				}
			}
