...
//		int r = 3;
		...
		System.out.println("Before 재귀 combList");
		...
		System.out.println("After 재귀  combList");
		for(int i=0; i<combList.size(); i++) {
			System.out.print(Arrays.toString(combList.get(i)) + "/");
		}
		System.out.println();
	...
	public static void combination(int n, int r, int[] selected, int index, int[] input, int target, ArrayList<int[]> combList, int limit) {
		if (combList.size() == limit) {
			System.out.println("원하는 최종결과...");
			return;
		}

		if (r == 0) {
			combList.add(selected.clone());
			System.out.println("combList 추가...");
			return;
		} else if (target == n) {
			System.out.println("3개 다 못뽑고 n개까지 탐색...");
			return;
		} else {
