...
public class _7_합쳐지는구슬들_1st_시뮬레이션_EMPTY_0_이용 {
	...
	static void Update(int x, int y, Marble new_marble) {
	    ...
	    num = next_grid[x][y].num;
	    weight = next_grid[x][y].weight;
	    move_dir = next_grid[x][y].dir;
	    ...
	    if(new_num > num) {
	        next_grid[x][y] = new Marble(new_num, weight + new_weight, new_dir);
	    }
	    // 기존 구슬의 우선순위가 더 높다면
	    // 무게만 더해집니다.
	    else {
	        next_grid[x][y] = new Marble(num, weight + new_weight, move_dir);
	    }
	}
