...
public class B17822_원판돌리기_double {
	...
	public static void main(String[] args) {
		...
		map = new double[N+1][M];
		//map[A][B] : A의 인덱스는 1~N 까지만 사용. B 는 0~M-1.
		for(int r=1; r<=N; r++) {
			for(int c=0; c<M; c++) {
				map[r][c] = sc.nextDouble();
			}
		}
	...
	private static void rotateOne(double[] arr, int dir, int count) {
	...
	private static boolean removeCloseAndSame() {
		...
		for(int r=1; r<=N; r++) {
			for(int c=0; c<M; c++) {
				if(map[r][c] != Double.MAX_VALUE && !visited[r][c]) {
					double base = map[r][c];
					...
					while(!q.isEmpty()) {
						...
						for(int k=0; k<4; k++) {
							...
							if(map[nb][np] == base && !visited[nb][np]) {
								...
								visited[nb][np] = true;
								map[nb][np] = Double.MAX_VALUE;
							}
						}
					}
					
					if(inPartial) {
						map[r][c] = Double.MAX_VALUE;
					}
